...
class BeamPath:
    ...
    def propagate(self, z=None):
        if z is None:
            z = self.target_z if self.target_z is not None else (
                self.components[-1].z if self.components else self.initial_z
            )
    ...
    def optimize_path(self, *args, units='m', options=None):
        labels = args[0::2]
        bounds = args[1::2]
        idx = [next(i for i, c in enumerate(self.components) if c.label == lab) for lab in labels]
    ...
    def choose_components(self, *cands, cost_func=None, verbose=False):
        ...
        for label, clist, bnds in zip(cands[0::3], cands[1::3], cands[2::3]):
            if clist is None:
                candidate_lists.append([next(c for c in self.components if c.label == label)])
            else:
        ...
        for combo in product(*candidate_lists):
            ...
            score = cost_func(bp) if cost_func else bp.propagate().overlap(bp.target_beam)
            paths.append(bp); scores.append(score)
            if verbose:
    ...
    def plot_summary(self, title=None):
        ...
        z_end = self.target_z if self.target_z is not None else (
            self.components[-1].z if self.components else self.initial_z
        )
        ...
        if title:
            fig.suptitle(title)
        ax.set_xlabel('z [m]'); ax.set_ylabel('w(z) [m]'); ax.legend(); ax.grid(True)
        plt.show()
